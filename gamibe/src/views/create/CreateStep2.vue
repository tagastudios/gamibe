        ...
        <div class="mb-8">
            ...
            <div v-if="isEarning" class="mx-auto flex w-full max-w-lg flex-col gap-8">
                ...
                <BaseDropdown
                    ...
                    control-add
                    control-remove
                    @control-add="addNewOption('earning', 'category', $event)"
                    @control-remove="removeOption('earning', 'category', $event)"
                />
                <BaseInput
            ...
            <div v-if="isBill" class="mx-auto flex w-full max-w-lg flex-col gap-8">
                ...
                <BaseDropdown
                    ...
                    control-add
                    control-remove
                    @control-add="addNewOption('bill', 'category', $event)"
                    @control-remove="removeOption('bill', 'category', $event)"
                />
...
const { options: fixedFrequencyOptions } = useFrequency()
...
const addNewOption = (type: string, key: string, value: string) => {
    if (type === 'earning') {
        earning[key as keyof typeof earning] = value as never
        if (key === 'category') profile.customLists.earningCategories = value
        else if (key === 'frequency') profile.customLists.earningFrequencies = value
    } else if (type === 'bill') {
        bill[key as keyof typeof bill] = value as never
        if (key === 'category') profile.customLists.billCategories = value
        else if (key === 'frequency') profile.customLists.billFrequencies = value
    }
}

const removeOption = (type: string, key: string, value: string) => {
    if (type === 'earning') {
        if (key === 'category') profile.customLists.removeArrayItem('earningCategories', value)
        else if (key === 'frequency')
            profile.customLists.removeArrayItem('earningFrequencies', value)
    } else if (type === 'bill') {
        if (key === 'category') profile.customLists.removeArrayItem('billCategories', value)
        else if (key === 'frequency') profile.customLists.removeArrayItem('billFrequencies', value)
    }
}

const categoryOptions: any = computed(() => {
    return {
        bill: [
            // Custom
            ...profile.customLists.billCategories,
            // Housing
            {
                id: 'housing',
                title: 'Housing',
            ...
            // Clothing
            {
                ...
                title: 'Clothing',
                description:
                    'Adults’ clothing, adults’ shoes, children’s clothing, children’s shoes.',
                image: ''
            },
            ...
            // Gifts/Donations
            {
                ...
                description:
                    'Birthday, anniversary, wedding, Christmas, special occasion, charities.',
                image: ''
            },
        ...
        earning: [
            // Custom
            ...profile.customLists.earningCategories,
            // Salary & Wages
            {
                id: 'salary',
                title: 'Salary & Wages',
            ...
            // Alimony received
            {
                ...
                description:
                    'Payment received from a former spouse as part of a divorce settlement.',
                image: ''
            },
...
const frequencyOptions: any = computed(() => {
    return {
        bill: [
            // Custom
            ...profile.customLists.billFrequencies,
            // Hard Coded
            ...fixedFrequencyOptions
        ],
        earning: [
            // Custom
            ...profile.customLists.earningFrequencies,
            // Hard Coded
            ...fixedFrequencyOptions
        ]
    }
